...
$('#save-changes-button').on('click', function() {
    ...
    if (previousDescription != description_val || $('#id_avatar').prop('files').length > 0 || $('#id_cover').prop('files').length > 0) {
        var formData = new FormData();
        formData.append("description",description_val)
        formData.append("csrfmiddlewaretoken",csrf_submit_form)
        if ($('#id_avatar').prop('files').length >0) {
            formData.append("avatar",$('#id_avatar')[0].files[0])
        }
        if ($('#id_cover').prop('files').length >0) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                messageHtml = `<p id="success-message" class="widget-box-title" style="color:green">Your account has been saved!</p>`
                $('#profile-widget-box').prepend(messageHtml)
            ...
            error: function(data) {
                ...
                $('#profile-widget-box').prepend(messageHtml)
                $('#success-message').show(3).delay(1000).hide(1);
            }
        })
    }
})
...
$('#cover-upload-box').click(function(){ 
    document.getElementById('id_cover').click()
});
